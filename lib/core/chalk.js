/**
 * lib/core/chalk.js — 公共终端样式工具模块
 *
 * 从 .agents/skills/report/scripts/core-lib/chalk.js 复制，
 * 原文件保持不变（双轨制原则）。
 *
 * 提供统一的 ANSI 颜色常量、图标、分隔线、spinner 动画、
 * 表格渲染等终端输出工具，供所有新 Skill 引用。
 *
 * 用法：
 ...
 * 打印错误信息。
 *
 * Phase 6 改进：不再直接调用 process.exit()，改为抛出 CliError 异常。
 * 调用方可在顶层 catch 中决定退出行为，实现"深层不退出"原则。
 *
 * @param {string} message - 错误消息
 * @param {object} [options]
 * @param {string} [options.hint] - 提示信息
 * @param {boolean} [options.exit=true] - 是否抛出异常（true=抛异常，false=仅打印不抛）
 * @throws {Error} 当 options.exit 不为 false 时抛出包含 message 的 Error
 */
function error(message, options = {}) {
  ...
  if (shouldExit) {
    // Phase 6: 抛出异常而非直接 process.exit，由调用方顶层处理退出
