...
## ✅ 生成代码前自检清单（开写前 30 秒过一遍）

> 本清单是下方各章节硬规则的速查聚合，**不含新增知识**，只是把散落的规则汇总成一张开写前可逐条勾选的表。生成任意自定义页面代码前先过一遍；命中任一 ❌ 先改正再继续。

**A. 语法致命项（违反 → 白屏 / 模块静默不加载，且控制台无报错）**
- [ ] 无 ES6 计算属性名 `{ [key]: value }`；动态键一律先建对象再 `obj[key] = value`（含 `searchFieldJson`、`setCustomState`）→ 见「JS 引擎兼容性限制 1」
- [ ] `.then()` 回调内无 `padStart` / `padEnd`；补零改用三元 `n < 10 ? '0' + n : '' + n` → 见「JS 引擎兼容性限制 2」
- [ ] 发布编译 Babel 用 `modules: 'commonjs'`，不是 `false` → 见「JS 引擎兼容性限制 3」

**B. 状态与渲染**
- [ ] 业务态只写 `_customState`，读只用 `getCustomState()`；`this.state` 仅读 `timestamp` / `urlParams`
- [ ] `this.setState` 只用于更新 `timestamp` 触发重渲染，不用它存业务态
- [ ] `forceUpdate()` 后不在同步代码里立即取新 DOM，需 `setTimeout` 延后 → 见「编注 14」
- [ ] 输入类控件用 `defaultValue` + `onChange` 静默写 `_customState`，不做 `value` 受控绑定 → 见「编注 9」

**C. 函数与事件**
- [ ] 需要 `this` 的方法一律 `export function`，不用 `const fn = () =>` / `function()` → 见「编注 1」
- [ ] `renderJsx` 顶部 `var self = this`，事件与数组回调用箭头函数包裹；禁裸传 `this.方法`、禁 `.bind(this)` → 见「编注 2」
- [ ] `didUnmount` 清理所有 `setInterval` / `setTimeout` → 见「编注 4」

**D. 数据接口**
- [ ] 分页 `pageSize` ≤ 100 → 见「编注 8」
- [ ] 所有 API 调用带 `.catch()`；失败 / 超时 / 结构异常路径必须把 `loading` 置回 `false` 并给空态或错误态 → 见「编注 5」
- [ ] 字段 ID 统一走顶部 `FIELDS` 常量，不散写字符串 → 见「编注 18」
- [ ] `DateField` 写毫秒时间戳，不写字符串 → 见「编注 10」
- [ ] `AttachmentField` / `ImageField` 走 OSS 直传链路，不直接写 File → 见「编注 10.1」

**E. 视觉与交互**
- [ ] 已完成 6 步视觉决策，未套「灰白 AI 味」→ 见「编注 0」
- [ ] UI 文案/标题/按钮不堆装饰性 emoji（✅🚀👍 等是典型 AI 味）→ lint W07 会提示，非功能必需一律移除
- [ ] 用户可见下拉用自定义组件，不用原生 `<select>`
- [ ] Tailwind 只用已验证的 `g.alicdn.com` / 企业自托管地址，且有 fallback 兜底样式
- [ ] 中文输入用 `_isComposing` 处理组合输入 → 见「编注 3」

**F. 发布前**
- [ ] 已用真实 Schema 核对 `FIELDS`（fieldId 来自系统配置清单，不猜）
- [ ] 保存表单后再查询验证字段确实写入
...
### 编注 0：代码生成前确认功能摘要 + 视觉决策

生成页面代码前，AI 必须先完成两步前置工作：

**第一步：视觉决策（6 步法）**

按照 [视觉决策指南](visual-decision-guide.md) 完成以下 6 步：
1. **信息架构梳理**：确定页面类型、内容优先级、数据来源、操作入口
2. **视觉方向选择**：从 5 套方向模板（商务专业/活力科技/温暖人文/极简效率/品牌定制）中选择一套
3. **信息密度配置**：选择紧凑/舒适/宽松模式
4. **导航壳选择**：选择无导航/顶部栏/Tab/左侧导航/底部Tab
5. **详情页样式注入**：选择卡片堆叠/左右分栏/描述列表/时间线/步骤条
6. **最终审查**：对照审查清单检查视觉一致性

> **此步骤不可跳过**——跳过会导致页面出现"统一灰白 AI 味"。
