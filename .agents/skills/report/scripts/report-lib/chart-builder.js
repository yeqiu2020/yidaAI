...
const { warn } = require('../core-lib/chalk');
...
function buildReportSchema(reportTitle, charts, reportId, cubeTenantId) {
  ...
  charts.forEach((chart, index) => {
    ...
    // 合并用户自定义 settings（支持浅层嵌合并）
    const defaultSettings = getChartSettings(chart.type);
    const customSettings = chart.settings || {};
    const mergedSettings = { ...defaultSettings, ...customSettings };
    if (customSettings.style && defaultSettings.style) {
      mergedSettings.style = { ...defaultSettings.style, ...customSettings.style };
    }
    if (customSettings.xAxis && defaultSettings.xAxis) {
      mergedSettings.xAxis = { ...defaultSettings.xAxis, ...customSettings.xAxis };
    }
    if (customSettings.yAxis && defaultSettings.yAxis) {
      mergedSettings.yAxis = { ...defaultSettings.yAxis, ...customSettings.yAxis };
    }
    if (customSettings.leftYAxis && defaultSettings.leftYAxis) {
      mergedSettings.leftYAxis = { ...defaultSettings.leftYAxis, ...customSettings.leftYAxis };
    }
    if (customSettings.rightYAxis && defaultSettings.rightYAxis) {
      mergedSettings.rightYAxis = { ...defaultSettings.rightYAxis, ...customSettings.rightYAxis };
    }
    if (customSettings.legend && defaultSettings.legend) {
      mergedSettings.legend = { ...defaultSettings.legend, ...customSettings.legend };
    }
    if (customSettings.label && defaultSettings.label) {
      mergedSettings.label = { ...defaultSettings.label, ...customSettings.label };
    }
