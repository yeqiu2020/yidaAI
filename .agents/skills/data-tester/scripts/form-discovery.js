    ...
    for (const line of lines) {
      ...
      if (inTable && trimmed.startsWith('|')) {
        ...
        if (cells.length >= 3) {
          ...
          const uuid = cells[2];
          const group = cells.length >= 5 ? cells[4] : null;

          const match = nameWithType.match(/^(.+?)「(.+?)」$/);
          if (match) {
            const name = match[1];
            const type = match[2];
            const formDirName = `${name}「${type}」`;
            const dir = group ? `${group}「分组」/${formDirName}` : formDirName;
            forms.push({
              name,
              type,
              uuid,
 ...
 */
function validateProjectConfig(projectDir) {
  const errors = [];
  const warnings = [];
  ...
  try {
    ...
    errors.push(`无法读取系统配置清单: ${configPath}`);
    return { valid: false, errors, warnings };
  }

  // 1. App ID 格式校验
  const appIdMatch = configContent.match(/APP_[A-Z0-9]+/);
  if (!appIdMatch) {
    errors.push('系统配置清单中未找到有效的应用ID（APP_开头）');
    return { valid: false, errors, warnings };
  }
  const appId = appIdMatch[0];
  if (!/^APP_[A-Z0-9]{16,}$/.test(appId)) {
    errors.push(`应用ID格式异常: ${appId}（长度过短，可能是无效ID）`);
  }

  // 2. 与「组织及应用信息.md」交叉校验（向上查找 + 已知工作区根目录）
  let orgInfoPath = null;
  let searchDir = projectDir;
  for (let i = 0; i < 4; i++) {
    const candidate = path.join(searchDir, '组织及应用信息.md');
    if (fs.existsSync(candidate)) {
      orgInfoPath = candidate;
      break;
    }
  ...
  if (orgInfoPath) {
    try {
      const orgContent = fs.readFileSync(orgInfoPath, 'utf-8');
      const validAppIds = new Set();
      const lines = orgContent.split('\n');
      for (const line of lines) {
        const m = line.match(/APP_[A-Z0-9]+/g);
        if (m) m.forEach(id => validAppIds.add(id));
      }
      if (validAppIds.size > 0 && !validAppIds.has(appId)) {
        errors.push(
          `🚨 安全校验失败！系统配置清单中的应用ID ${appId} 不在「组织及应用信息.md」的已知应用列表中。` +
          `这可能是因为指向了旧/外部目录，有效ID应为: ${[...validAppIds].join(', ')}。` +
          `请检查工作区路径是否正确，切勿使用外部旧目录。`
        );
      }
    } catch (e) {
      warnings.push(`读取组织及应用信息失败: ${e.message}`);
    }
  } else {
    warnings.push('未找到「组织及应用信息.md」，跳过应用ID交叉校验');
  }

  // 3. 表单 UUID 格式校验
  const uuidPattern = /FORM-[A-Z0-9]{20,}/g;
  const uuids = configContent.match(uuidPattern) || [];
  if (uuids.length === 0) {
    errors.push('系统配置清单中未发现任何有效的表单UUID（FORM-开头）');
  } else {
    const invalidUuids = uuids.filter(u => !/^FORM-[A-Z0-9]{20,40}$/.test(u));
    if (invalidUuids.length > 0) {
      warnings.push(`部分表单UUID格式异常: ${invalidUuids.join(', ')}`);
    }
  }
