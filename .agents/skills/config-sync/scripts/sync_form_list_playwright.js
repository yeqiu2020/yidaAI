...
function createFormFolder(projectDir, form, visualMode) {
  try {
    // v1.8.0: 优先使用从宜搭API获取的分组信息（form.module）
    // 如果没有API分组信息，使用硬编码的moduleMap作为fallback（向后兼容）
    const moduleMap = {
      '产品信息': '02基础信息',
      '仓库信息': '02基础信息',
      '库存盘点': '03库存管理',
    ...
    // 优先使用API获取的分组，其次使用硬编码映射
    const moduleName = form.module || moduleMap[form.name] || null;
...
const cookiePath = cookieFile;
    ...
    // 域名匹配函数
    const isDomainMatch = (cookieDomain, host) => {
    ...
    const filteredCookies = targetHost
      ? allCookies.filter(c => isDomainMatch(c.domain, targetHost))
      : allCookies;
    ...
    // 提取 csrf token
    const csrfCookie = Array.from(uniqueByName.values()).find(c => c.name === 'csrfToken');
    const csrfToken = csrfCookie ? csrfCookie.value : '';

    return { cookieStr, csrfToken };
  } catch (error) {
    console.log(`⚠️ 读取Cookie失败: ${error.message}`);
    return { cookieStr: '', csrfToken: '' };
  }
}
...
/**
 * 调用宜搭导航API获取表单分组信息
 * v1.8.0: 新增函数，通过 getFormNavigationListByOrder.json API 获取分组
 * @returns {Object} formUuid -> 分组名称 的映射
 */
async function fetchFormNavigation(appId, baseUrl) {
  const formToGroup = {};

  try {
    const { cookieStr, csrfToken } = loadCookieString(baseUrl);
    if (!cookieStr) {
      console.log('⚠️ 无可用Cookie，跳过获取分组信息');
      return formToGroup;
    }

    const apiUrl = `${baseUrl}/dingtalk/web/${appId}/query/formnav/getFormNavigationListByOrder.json?_api=Nav.queryList&_mock=false`;
    console.log(`📡 调用导航API获取分组信息...`);
    ...
    if (!result?.success || !Array.isArray(result.content)) {
      console.log(`⚠️ 获取导航分组失败: success=${result?.success}, errorCode=${result?.errorCode}`);
      return formToGroup;
    }

    // 解析导航树：先构建 navUuid → 分组名称 的映射
...
function updateConfigFile(projectDir, appId, appName, forms, added, removed, visualMode) {
  ...
  // 构建新的表单列表表格（包含流程Code列和分组列）
  const formTableHeader = `| 序号 | 页面名称「类型」 | 表单UUID | 流程Code | 分组 |
|------|-----------------|----------|----------|------|`;
  ...
  const formRows = forms.map((form, index) => {
    ...
    return `| ${index + 1} | ${form.name}「${typeStr}」 | ${form.formUuid} | ${processCode} | ${moduleName} |`;
  }).join('\n');
...
async function main() {
  try {
    ...
    // v1.8.0: 获取导航分组信息并合并到表单数据
    const baseUrl = 'https://qfhefh.aliwork.com';
    const formToGroup = await fetchFormNavigation(appId, baseUrl);
    if (Object.keys(formToGroup).length > 0) {
      ...
      const groupedForms = remoteForms.filter(f => f.module).length;
      printInfo(`分组信息: ${groupedForms}/${remoteForms.length} 个表单有分组`, visualMode);
    }
