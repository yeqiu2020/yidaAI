 ...
 * 查找项目根目录（project 工作区）。
 *
 * 查找策略（阶段二改造：优先 cwd，取代 __dirname 上溯）：
 *   1. process.cwd()（当前工作目录，CLI 模式下即用户项目根）
 *   2. 通过环境变量检测当前活跃的 AI 工具的工作区
 *   3. 兼容回退：lib/core → ../..（包根，开发模式）
 *
 * @returns {string} 项目根目录的绝对路径
 */
function findProjectRoot() {
  // 优先 process.cwd()（CLI 模式下即用户工作目录）
 ...
 * 读取登录态缓存。
 *
 * 阶段二改造：优先全局 Cookie（~/.yida-ai-helper/.cookies.json），
 * 兼容项目根 .cookies.json 和 .cache/cookies.json（老项目不断档）。
 *
 * 优先级（高 → 低）：
 *   1. 全局 ~/.yida-ai-helper/.cookies.json
 *   2. 项目根 .cookies.json
 *   3. 项目根 .cache/cookies.json
 *
...
function loadCookieData(projectRoot, defaultBaseUrl) {
  ...
  // 1. 全局 Cookie（全局共享登录态；尊重 YIDA_HELPER_HOME 与 paths.js 保持一致，供测试隔离）
  const globalHome = process.env.YIDA_HELPER_HOME || path.join(os.homedir(), '.yida-ai-helper');
  const globalCookieFile = path.join(globalHome, '.cookies.json');
  // 2. 项目根 Cookie（老项目兼容）
  const rootCookieFile = path.join(root, '.cookies.json');
  // 3. 旧格式 .cache/cookies.json
