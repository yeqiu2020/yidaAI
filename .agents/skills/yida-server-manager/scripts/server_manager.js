...
async function forceStopPorts() {
  ...
  if (httpPidAfter) {
    ...
    try { execSync(`taskkill /PID ${httpPidAfter} /F /T`, { stdio: 'ignore' }); } catch(e) {}
    await new Promise(r => setTimeout(r, 500));
  }

  if (syncPidAfter) {
    logStep('0', `⚠️ 端口 ${CONFIG.syncPort} 仍被占用 (PID: ${syncPidAfter})，强制终止...`, 'red');
    try { execSync(`taskkill /PID ${syncPidAfter} /F /T`, { stdio: 'ignore' }); } catch(e) {}
    await new Promise(r => setTimeout(r, 500));
  }
...
async function stopAll() {
  ...
  if (fs.existsSync(CONFIG.builtInServerScript)) {
    try { fs.unlinkSync(CONFIG.builtInServerScript); } catch(e) {}
  }
