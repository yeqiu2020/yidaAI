...
// 依赖的 skill 路径
const CONFIG_SYNC_SCRIPT = path.join(PROJECT_ROOT, '.agents', 'skills', 'config-sync', 'scripts', 'sync_config.js');
const SCHEMA_SYNC_SCRIPT = path.join(PROJECT_ROOT, '.agents', 'skills', 'get-schema', 'scripts', 'sync-schema.js');
const RULE_SYNC_SCRIPT = path.join(PROJECT_ROOT, '.agents', 'skills', 'rule-sync', 'scripts', 'sync_rules.js');
...
function checkDependencies() {
  const deps = [
    { path: CONFIG_SYNC_SCRIPT, name: 'config-sync' },
    { path: SCHEMA_SYNC_SCRIPT, name: 'get-schema' },
    { path: RULE_SYNC_SCRIPT, name: 'rule-sync' }
  ];
...
async function syncSchema(appId, outputDir, forms) {
  ...
  for (let i = 0; i < forms.length; i++) {
    ...
    ensureDir(formDir);
    const dirLabel = groupDirName ? `${groupDirName}/${formDirName}` : formDirName;
    console.log(`   📁 表单目录: ${dirLabel}`);
...
/**
 * 从系统配置清单解析表单列表
 * 支持格式: | 序号 | 页面名称「类型」 | 页面编码（表单UUID） | ... | 所属分组 |
 * v1.1.0: 解析第5列分组信息，支持按分组创建本地目录
 */
function parseFormsFromConfig(configPath) {
  ...
  for (const rawLine of lines) {
    ...
    // v1.1.0: 解析第5列分组信息（cells[4]），兼容3列和4列格式
    const module = cells.length >= 5 ? (cells[4] && cells[4] !== '-' ? cells[4].trim() : null) : null;
